...
| `config_flow.py` | `ElectricIrelandInsightsConfigFlow` | Steps: `user` (credentials) → `account` (selection) → create entry. `reauth_confirm` for expired passwords. `reconfigure` for password change + meter ID rediscovery. |
| `sensor.py` | `ElectricIrelandDiagnosticSensor`, `DIAGNOSTIC_SENSORS`, `PARALLEL_UPDATES = 0` | Two `CoordinatorEntity` subclasses: last import timestamp + data freshness days. Disabled by default. EntityCategory.DIAGNOSTIC. |
...
| `diagnostics.py` | `async_get_config_entry_diagnostics` | Redacts: username, password, account_number, partner, contract, premise. Exposes coordinator data for debugging. |
